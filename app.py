...
from kyungdong_agent import (
    ...
    create_repository, KyungdongToolRegistry,
    ManufacturingAgent, timestamp, user_question_history,
)
...
repository = create_repository(BASE_DIR / "data" / "kyungdong_demo.db")
registry = KyungdongToolRegistry(repository)
...
with st.sidebar:
    ...
    st.divider()
    st.caption("데모 데이터·예측은 실제 운영값이 아닙니다. 견적·발주·대체자재·FAT·출하는 담당자가 승인합니다.")
...
<header class="factory-header">
  <div><div class="brand-line">KYUNGDONG GLOBALTECH / MANUFACTURING</div>
  <h1>경동글로벌텍 AI Agent</h1>
  <p>금속가공의 모든 공정, 하나의 연결된 작업 공간.</p></div>
  <div class="header-meta"><strong>생산관리 워크스페이스</strong><span>검증용 데모 데이터</span></div>
</header>
<div class="process-strip" aria-label="제조 업무 흐름">
  <b>제조 공정</b><span>수주·견적</span><i>→</i><span>설계</span><i>→</i>
  <span>자재·구매</span><i>→</i><span>레이저·포밍</span><i>→</i>
  <span>용접·조립</span><i>→</i><span>FAT 검사</span><i>→</i><span>출하·설치</span>
</div>
<div class="overview-label"><strong>현장 주요 지표</strong><span>프로젝트 / 자재 / 품질</span></div>
""", unsafe_allow_html=True)

with st.container(key="kpis"):
    metrics = st.columns(5)
    metrics[0].metric("진행 프로젝트", f"{kpi['active_projects']}건")
    metrics[1].metric("자재 이슈", f"{kpi['material_issues']}건")
    metrics[2].metric("고위험 발주", f"{kpi['po_risks']}건")
...
with context_col:
    ...
    with st.container(border=True, key="context"):
        ...
        if documents:
            query = st.text_input("문서 검색", placeholder="예: 자재, 도면, FAT", key="document_query")
...
st.markdown('<footer class="workspace-footer">경동글로벌텍 제조 업무 지원 | 표시된 수치와 예측은 검증용 가상 데이터입니다.</footer>', unsafe_allow_html=True)
